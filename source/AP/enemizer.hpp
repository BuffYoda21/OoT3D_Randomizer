...
class EnemyType {
  public:
    EnemyType() = default;
    EnemyType(std::string _name, bool* _soulVar, std::vector<u16> _possibleParams, std::vector<LocType> _validLocTypes)
        : name(std::move(_name)), soulVar(_soulVar), possibleParams(_possibleParams),

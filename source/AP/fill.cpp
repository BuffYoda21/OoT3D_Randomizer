...
static void RandomizeOwnDungeon(const Dungeon::DungeonInfo* dungeon) {
    ...
    std::vector<LocationKey> dungeonLocations;
    ...
    */
    ...
    // Hopefully this will work fine as an alternative since HintRegions are being nuked
    for (Dungeon::DungeonInfo* dungeon : dungeonList) {
        for (LocationKey loc : dungeon->GetDungeonLocations()) {
            dungeonLocations.push_back(loc);
        }
    }
...
int Fill() {
    ...
    while (retries < 5) {
        ...
        if (playthroughBeatable && !placementFailure) {
            printf("Done\n");
            printf("Calculating Playthrough...");
            PareDownPlaythrough();
            CalculateWotH();
            printf("Done\n");
            CreateItemOverrides();
            ...
            // CreateAlwaysIncludedMessages();
            // CreateMiscHints();
            /** hints will be handled by archipelago

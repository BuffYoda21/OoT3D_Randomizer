...
void InitLogDirectories() {
    ...
    const std::string stylesheetSrc  = "spoiler-log.css"; // local file
    const std::string stylesheetDest = "OoT3DR/Logs/spoiler-log.css";

    if (fs::exists("OoT3DR/Logs")) {
        return;
    }
    ...
    */
    ...
    try {
        fs::copy_file(stylesheetSrc, stylesheetDest, fs::copy_options::overwrite_existing);
    } catch (...) { printf("Failed to copy stylesheet\n"); }
}
...
static void WriteEnabledTricks(tinyxml2::XMLDocument& spoilerLog) {
    ...
    for (const auto& setting : Settings::trickOptions) {
        if (setting->GetSelectedOptionIndex() != TRICK_ENABLED) {
            continue;
        }
...
static void WritePlaythrough(tinyxml2::XMLDocument& spoilerLog, const bool collapsible = false) {
    ...
    for (int i = 0; i < playthroughLocations.size(); ++i) {
        if (collapsible) {
...
static void WriteShuffledEntrances(tinyxml2::XMLDocument& spoilerLog, const bool collapsible = false) {
    ...
    for (int i = 0; i < playthroughEntrances.size(); ++i) {
        if (collapsible) {
            playthroughNode->InsertEndChild(CreateCollapseCheckbox(spoilerLog));
        }
        auto sphereNode = playthroughNode->InsertNewChildElement("sphere");
